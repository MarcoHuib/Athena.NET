...
internal static class MapInventoryEquipUpdateProtocol
{
    ...
    internal static bool TryParseRequest(ReadOnlySpan<byte> p,out uint a,out uint c,out uint slotIndex,out uint equip)
    {
        a=0;c=0;slotIndex=0;equip=0;
        ...
        slotIndex=BinaryPrimitives.ReadUInt32LittleEndian(p[10..]);
        equip=BinaryPrimitives.ReadUInt32LittleEndian(p[14..]);
        return true;
    }
    internal static byte[] BuildResponse(bool success,uint charId,uint slotIndex)
    {
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(p.AsSpan(7),slotIndex);
        return p;
    }
}
