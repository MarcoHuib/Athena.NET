...
public sealed class MapInventoryConsumeProtocolTests
{
    ...
    {
        // opcode 0x2b37 LE, accountId=2000000(0x1E8480), charId=9, slotIndex=2, amount=1
        byte[] packet = [0x37, 0x2b, 0x80, 0x84, 0x1e, 0x00, 9, 0, 0, 0, 2, 0, 0, 0, 1, 0, 0, 0];

        Assert.True(MapInventoryConsumeProtocol.TryParseRequest(packet, out var accountId, out var charId, out var slotIndex, out var amount));
    ...
    {
        var packet = MapInventoryConsumeProtocol.BuildResponse(success: true, charId: 9, slotIndex: 2, newAmount: 0, rowDeleted: true);
    ...
    {
        var packet = MapInventoryConsumeProtocol.BuildResponse(success: true, charId: 9, slotIndex: 2, newAmount: 4, rowDeleted: false);
    ...
    {
        var packet = MapInventoryConsumeProtocol.BuildResponse(success: false, charId: 9, slotIndex: 2, newAmount: 0, rowDeleted: false);
