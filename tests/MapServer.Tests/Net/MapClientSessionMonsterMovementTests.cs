...
// monster tick loop in production - see that method's own doc comment): WalkStarted and
// WalkFinished each send exactly one capture-verified packet (0x09FD / 0x0088) to an
// already-visible session, CellCrossed sends NOTHING (pinned unit_walktoxy_nextcell's ordinary
// per-cell continuation never resends the walk packet), a monster that just walked into a
// stationary session's own 14-cell range gets a fresh discovery packet exactly once (0x09FD if
// still walking, 0x09FF if not), and a monster still out of range or on a different map gets
// nothing.
public sealed class MapClientSessionMonsterMovementTests
{
    ...
    {
        // No real-time wait is needed: nothing server-side can produce more bytes without another
        // client request - a ping round-trip proves the server has caught up with everything sent
        // so far without a spurious extra packet in between.
    ...
    [Fact]
    public async Task NotifyMonsterMovedAsync_WalkFinished_AlreadyVisible_SendsStopMoveOnTheWire()
    {
        var (client, stream, session, run, target) = await SetupAsync();
        using var _ = client;
        await MakeVisibleAsync(stream, target);
        ...
        Assert.True(target.TryStartIdleWalk([(75, 51), (76, 51)], orthogonalStepMs: 400, now: 1, nowOffset: DateTimeOffset.UnixEpoch, jitterMs: () => 0));
        // Actually finish the walk so the reported position matches a real WalkFinished's final cell.
        ...
        var stopPacket = await ReadExact(stream, PacketConstants.ZcStopMoveLength);
        Assert.Equal((short)PacketConstants.ZcStopMove, BinaryPrimitives.ReadInt16LittleEndian(stopPacket));
        Assert.Equal(target.ActorId, BinaryPrimitives.ReadUInt32LittleEndian(stopPacket.AsSpan(2)));
        Assert.Equal((ushort)76, BinaryPrimitives.ReadUInt16LittleEndian(stopPacket.AsSpan(6)));
        Assert.Equal((ushort)51, BinaryPrimitives.ReadUInt16LittleEndian(stopPacket.AsSpan(8)));
    ...
    public async Task KillThenRespawn_InsideVisibilityRange_MakesTheInstanceVisibleAgain_WithoutWaitingForIdleWalk()
    {
        ...
        attackPacket[7] = 0x7f;
        await stream.WriteAsync(attackPacket);

        var damagePacket = await ReadExact(stream, PacketConstants.ZcNotifyAct3Length).WaitAsync(TimeSpan.FromSeconds(5));
        Assert.Equal((short)PacketConstants.ZcNotifyAct3, BinaryPrimitives.ReadInt16LittleEndian(damagePacket));
        var vanishPacket = await ReadExact(stream, PacketConstants.ZcNotifyVanishLength).WaitAsync(TimeSpan.FromSeconds(5));
