...
internal static class MobDataCompiler
{
    ...
    // physical file per map/world-family module (e.g. "PrtFild08Spawn.cs", "PayFild01Spawn.cs",
    // "EvtZombieSpawn.cs") rather than the earlier source-file-sharded "<className>.<Suffix>.cs"
    // shape - every generated spawn file's own basename ends with the fixed "Spawn.cs" suffix,
    // distinguishing it unambiguously from hand-maintained sibling files in the same folder
    // (*Npcs.cs/*Warps.cs/*World.cs, Scripts/) which never end that way. The registry itself is
    // named literally `registryFileName` (e.g. "GeneratedMobSpawnRegistry.cs"). Both still require
    // the standard auto-generated header before deletion - filename pattern alone is not proof of
    // ownership.
